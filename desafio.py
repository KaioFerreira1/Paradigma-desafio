def calcular_soma_calibracao(documento):
    soma = 0
    for linha in documento:
        digitos = [char for char in linha if char.isdigit()]
        if digitos:
            valor_calibracao = int(digitos[0] + digitos[-1])
            soma += valor_calibracao
    return soma
...
soma_calibracao = calcular_soma_calibracao(documento_exemplo)
print("A soma dos valores de calibração é:", soma_calibracao)
